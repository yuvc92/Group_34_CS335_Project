...
%token<str> EQ_OP NEQ_OP LEQ LSHIFT GEQ 
%token<str> RSHIFT ADDASG SUBASG MULASG DIVASG MODASG 
%token<str> EXP DOUBLEDIV DOUBLEMULASG DOUBLEDIVASG 
%token<str> COLON COMMA SEMICOLON PLUS MINUS MUL DIVIDE PIPE AMPERSAND LESS 
%token<str> NEWLINE  
%token<str> NUMBER SPACE  BACKSPACE TSTRING  STRING  
%token<str> NAME WS END

%token<str> AND ASSERT BREAK CLASS CONTINUE DEF DEL ELIF ELSE FOR FROM GLOBAL
%token<str> IF IMPORT IN IS  NOT OR PASS PRINT RETURN TRY WHILE WITH YIELD
...
%MULt translation_unit
%%

	file_input :	single_stmt END{
		$$ = assign($1);
		string str = ""
...
//or_test(p)
		...
		$$ = assign($1);
			...
			| xor_expr PIPE expr{
				if($1->type == "NUMBER" && $3->type == "NUMBER"){
					...
					$$->place = getNewTempVar();
					$$->type = "NUMBER";
					emit(getCurrScope(), $$->place, $1->place, $3->place, "PIPE");
				}
				else {
					if($1-> type == "REFERENCE_ERROR" || $3-> type == "REFERENCE_ERROR")error();
				}
	...
	and_expr 	: shift_expr{
		$$ = assign($1);
				...
				if($1->type == "NUMBER" && $3->type == "NUMBER"){
					...
					$$->place = getNewTempVar();
					$$->type = "NUMBER";
					emit(getCurrScope(), $$->place, $1->place, $3->place, "AMPERSAND");
				}
				else {
					if($1-> type == "REFERENCE_ERROR" || $3-> type == "REFERENCE_ERROR")error();
				}
	...
	shiftexprlist 	:
						 AMPERSAND shift_expr shiftexprlist
	;

	shift_expr 	: arith_expr {
		$$$ = assign($1);
	}
				| arith_expr LSHIFT shift_expr{
				if($1->type == "NUMBER" && $3->type == "NUMBER"){
					$$ = makenode();
					$$->place = getNewTempVar();
					$$->type = "NUMBER";
					emit(getCurrScope(), $$->place, $1->place, $3->place, "LSHIFT");
				}
				else {
					if($1-> type == "REFERENCE_ERROR" || $3-> type == "REFERENCE_ERROR")error();
				}
			}
				| arith_expr RSHIFT shift_expr{
				if($1->type == "NUMBER" && $3->type == "NUMBER"){
					$$ = makenode();
					$$->place = getNewTempVar();
					$$->type = "NUMBER";
					emit(getCurrScope(), $$->place, $1->place, $3->place, "RSHIFT");
				}
				else {
					if($1-> type == "REFERENCE_ERROR" || $3-> type == "REFERENCE_ERROR")error();
				}
	...
	arithexprlist 	:
						 LSHIFT arith_expr arithexprlist
						| RSHIFT arith_expr arithexprlist
	;
		...
		$$ = assign($2);
			...
			| factor MUL term{
				if($1->type == "NUMBER" && $3->type == "NUMBER"){
					...
					$$->place = getNewTempVar();
					$$->type = "NUMBER";
					emit(getCurrScope(), $$->place, $1->place, $3->place, "MUL");
				}
				else {
					if($1-> type == "REFERENCE_ERROR" || $3-> type == "REFERENCE_ERROR")error();
				}
			...
			| factor DIVIDE term{
				if($1->type == "NUMBER" && $3->type == "NUMBER"){
					...
					$$->place = getNewTempVar();
					$$->type = "NUMBER";
					emit(getCurrScope(), $$->place, $1->place, $3->place, "DIVIDE");
				}
				else {
					if($1-> type == "REFERENCE_ERROR" || $3-> type == "REFERENCE_ERROR")error();
				}
	...
	factorlist 	:
					 MUL factor factorlist
					| DIVIDE factor factorlist
					| '%%' factor factorlist
					| DOUBLESEMICOLONfactor factorlist
	;
